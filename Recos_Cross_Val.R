...
comp_rat_Ibcf <- cross_val_comp(r_matrix =  ratings_mat, test_sp,k_cross, k_val,gn,
                                recMethod = "IBCF", predType,nPred)

comp_rat_Ubcf <- cross_val_comp(r_matrix =  ratings_mat, test_sp,k_cross, k_val,gn,
               recMethod = "UBCF", predType,nPred)

popular <- cross_val_comp(r_matrix =  ratings_mat, test_sp,k_cross, k_val,gn,
                          recMethod = "Popular", predType,nPred)

random <- cross_val_comp(r_matrix =  ratings_mat, test_sp,k_cross, k_val,gn,
                         recMethod = "Random", predType,nPred)


f_cross <- evaluationScheme(binary_mat, method="cross", train=test_sp,
                            k=k_val, given = gn, goodRating = 0)

binary_rat_Ubcf <- cross_val_comp(r_matrix =  binary_mat, test_sp,f_cross, k_val,gn,
                                recMethod = "UBCF", predType,nPred)

binary_rat_Ibcf <- cross_val_comp(r_matrix =  binary_mat, test_sp,f_cross, k_val,gn,
                                  recMethod = "IBCF", predType,nPred)

overall_results <- data.frame(comp_rat_Ubcf)

overall_results <- rbind.data.frame(overall_results, 
                                    binary_rat_Ubcf,
                                    comp_rat_Ibcf,
                                    binary_rat_Ibcf,
                                    popular, 
...
colnames(overall_results) <- c("Comp_UBCF","Bin_UBCF",
                               "Comp_IBCF","Bin_IBCF",
                               "Popular", "Random")

overall <- data.frame(t(overall_results))[,c(1,2)]

overall <- cbind.data.frame(rownames(overall),overall)

colnames(overall)[1] <- "Method"

file_name <- paste("Recos_overall_IBCF", "_results.csv", sep = "")

write.csv(overall,file_name, row.names = FALSE)
...
comp_rat_Ubcf_cv <- cross_val_bin_comp(ratings_mat,k_cross,bins)
comp_rat_Ubcf_cv[,1] <- bins
comp_rat_Ubcf_cv <- comp_rat_Ubcf_cv[,c(1,4)]
